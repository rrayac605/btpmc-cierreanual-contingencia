...
public class AuditTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
        String action = stepExecution.getJobExecution().getExecutionContext().getString(ConfrontaConstants.ACTION_AUDIT_PARAM);
        Long key = stepExecution.getJobParameters().getLong(ConfrontaConstants.KEY_PARAM);
        processAuditRepository.createCorrect(action, key);
