...
@Component
@StepScope
public class BackupChangeProcessorContingencia implements ItemProcessor<ChangeDTO, BackupConfrontaDTO> {
    ...
    public BackupConfrontaDTO process(ChangeDTO changeDTO) throws Exception {
        ...
        backupConfrontaDTO.setNss(StringUtils.safeSubString(changeDTO.getNumNss(), ConfrontaConstants.TEN));
        ...
        backupConfrontaDTO.setRegPatronal(StringUtils.safeSubString(changeDTO.getRefRegistroPatronal(), ConfrontaConstants.TEN));
        ...
        if(changeDTO.getAudits() != null && changeDTO.getAudits().size()>0 ) {
        	backupConfrontaDTO.setClaveAccionRegistro(changeDTO.getAudits().get(changeDTO.getAudits().size()-1).getCveSituacionRegistro());
            backupConfrontaDTO.setDesAccionRegistro(changeDTO.getAudits().get(changeDTO.getAudits().size()-1).getDesSituacionRegistro());            
        }
